...
export class HanamiCommonComputationService implements HanamiCommonComputationPort {
	...
	private async dbGlobalPopularFallbackCandidates(): Promise<RawCandidate[]> {
	...
	private async dbRecentExplorationFallbackCandidates(): Promise<RawCandidate[]> {
		const sinceId = this.idService.gen(Date.now() - DB_GLOBAL_FALLBACK_WINDOW_MS);
		const rows = await this.db.query(
		...
		return rows.map((row, index) => ({ noteId: row.noteId, baseScore: (count - index) / count }));
	}
	...
	public async buildSourceBundle(input: HanamiCommonSourceBuildInput): Promise<HanamiCommonSourceBundle> {
		...
		const [trend, globalPopularRaw, explorationRaw] = await this.runSourcePhase(input.signal, () => {
			...
			const globalPopularPromise = !enabled.has('globalPopular') || featured.length > 0
				...
				: this.dbGlobalPopularFallbackCandidates();
			const explorationPromise = !enabled.has('exploration') || featured.length > 0
				? Promise.resolve<RawCandidate[]>([...featured]
					.sort((a, b) => a.noteId < b.noteId ? 1 : a.noteId > b.noteId ? -1 : 0)
					.map((candidate, index) => ({ noteId: candidate.noteId, baseScore: 1 - (index / FEATURED_HEADROOM) })))
				: this.dbRecentExplorationFallbackCandidates();
		...
		const candidateIds = [...new Set([
			...
			...(enabled.has('exploration') ? explorationRaw.map(candidate => candidate.noteId) : []),
		...
		const candidates: HanamiCommonCandidateMap = Object.freeze({
			globalPopular: enabled.has('globalPopular') ? this.finalizeCandidates(globalPopularRaw, safeAuthors, 'globalPopular', GLOBAL_POPULAR_POOL) : Object.freeze([]),
			trending: enabled.has('trending') ? this.finalizeCandidates(trendingRaw, safeAuthors, 'trending', TRENDING_POOL) : Object.freeze([]),
			exploration: enabled.has('exploration') ? this.finalizeCandidates(explorationRaw, safeAuthors, 'exploration', EXPLORATION_POOL) : Object.freeze([]),
		});
