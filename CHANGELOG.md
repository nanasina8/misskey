...
### Server
- Fix: はなみTLの発見枠のノート判定ジョブがCPU環境の35分タイムアウトに収まらず全損する問題を修正  
  既定バッチを64→16件に縮小（`HANAMI_NOTE_JUDGE_BATCH_SIZE` で 1..64 に調整可能）し、判定結果を1件ごとに逐次書き出すことで、タイムアウトやクラッシュ時も済んだ分を保存するようにしました
-
